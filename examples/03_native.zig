//! Phase 8 smoke test: native AppKit backend (no GLFW) with animated clear.
//!
//! Run with `zig build run-03_native`. Press Escape or close the window to
...
fn onWindowEvent(ctx: ?*anyopaque, event: zgpui.platform.WindowEvent) void {
    ...
    switch (event) {
        .input => |input| switch (input) {
            ...
            .scroll => |e| std.log.info("scroll delta ({d:.2}, {d:.2}) unit={s}", .{ e.delta.x, e.delta.y, @tagName(e.unit) }),
            .key_down => |e| {
...
pub fn main(init: std.process.Init) !void {
    ...
    const appkit = try zgpui.appkit_platform.AppKitPlatform.init(gpa);
    const plat = appkit.platform();
    defer plat.deinit();

    const window = try plat.openWindow(.{
        .title = "zgpui (native AppKit)",
