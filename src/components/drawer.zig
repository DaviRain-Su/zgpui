//! Headless drawer: edge slide-over panel with modal backdrop, Escape dismiss,
//! and optional focus trapping.
//!
//! Swipe gestures and snap points are deferred — open/closed only for now.
//! Shares overlay/backdrop patterns with `sheet.zig` but marks the panel as
...
pub const Props = struct {
    ...
    /// Reserved for future snap-point behavior; ignored for now.
    snap: bool = false,
...
pub fn openDrawer(app: *App, open: OpenValue) void {
    open.set(app, true);
}
...
/// Zero-size main-tree placeholder; registers the drawer overlay when open.
pub fn drawer(arena: std.mem.Allocator, props: Props) !*Div {
...
const DrawerFixture = struct {
    ...
    fn render(ctx: ?*anyopaque, arena: std.mem.Allocator, harness: *testing_mod.Harness) anyerror!element.Element {
        const self: *DrawerFixture = @ptrCast(@alignCast(ctx.?));
    ...
    fn openClick(ctx: ?*anyopaque, _: *const platform.MouseButtonEvent) void {
        const self: *DrawerFixture = @ptrCast(@alignCast(ctx.?));
        openDrawer(&self.harness.app, .{ .uncontrolled = self.open_state });
    }
};

test "drawer opens and closes via Escape" {
    var harness = testing_mod.Harness.init(std.testing.allocator, .{ .width = 400, .height = 300 });
    defer harness.deinit();

    var fixture = DrawerFixture{ .harness = &harness };
    fixture.open_state = try harness.app.new(OpenValue.Store, .{ .value = false });
    try harness.setRoot(&fixture, DrawerFixture.render);
    ...
    try harness.clickOn("open-drawer");
    try std.testing.expect(isOpen(&harness.app, .{ .uncontrolled = fixture.open_state }));
...
test "drawer closes via backdrop click" {
    var harness = testing_mod.Harness.init(std.testing.allocator, .{ .width = 400, .height = 300 });
    defer harness.deinit();
