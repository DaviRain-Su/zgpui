//! Headless preview card: non-modal link-preview overlay shown after trigger
//! hover delay, stays open while the pointer is over the trigger or card,
//! dismissed on leave (optional delay) or Escape.
...
pub const Props = struct {
    ...
    trigger_id: []const u8,
    state: app_mod.Entity(PreviewCardState),
...
const Host = struct {
    ...
    fn render(ctx: ?*anyopaque, arena: std.mem.Allocator) anyerror!element.Element {
        const self: *Host = @ptrCast(@alignCast(ctx.?));
        const visible = self.app.read(PreviewCardState, self.state).visible;
        if (!visible) {
        ...
        if (anchor) |bounds| {
            ...
            panel.style = s;
            const s_state = self.app.read(PreviewCardState, self.state);
...
const TriggerHoverHost = struct {
    ...
    fn onHover(ctx: ?*anyopaque, hovered: bool) void {
        ...
        const s = self.app.read(PreviewCardState, self.state);
...
fn storeCardBounds(app: *App, state: app_mod.Entity(PreviewCardState), panel: *const Div, anchor: Bounds(Pixels)) void {
    const s = app.read(PreviewCardState, state);
...
fn pointerOverCard(props: Props) bool {
    ...
    const s = props.app.read(PreviewCardState, props.state);
...
fn registerOverlay(arena: std.mem.Allocator, props: Props) !void {
    const visible = props.app.read(PreviewCardState, props.state).visible;
    if (!visible) return;
...
/// Attach hover handlers and register the preview-card overlay when visible.
/// Returns the same `trigger` div (with `onHover` wired).
pub fn previewCardWithTrigger(
...
pub fn hide(app: *App, state: app_mod.Entity(PreviewCardState)) void {
    const s = app.read(PreviewCardState, state);
    s.visible = false;
...
const PreviewCardFixture = struct {
    harness: *testing_mod.Harness = undefined,
    state: app_mod.Entity(PreviewCardState) = undefined,

    fn render(ctx: ?*anyopaque, arena: std.mem.Allocator, harness: *testing_mod.Harness) anyerror!element.Element {
        const self: *PreviewCardFixture = @ptrCast(@alignCast(ctx.?));

        var trigger = div_mod.div(arena)
            .withId("preview-card-trigger")
            .sizePx(80, 30)
            .bg(Rgba.fromHex(0x336699));
        trigger = try previewCardWithTrigger(arena, .{
            .id = "link-preview-card",
            .trigger_id = "preview-card-trigger",
            .state = self.state,
...
test "preview card shows after delay on trigger hover" {
    var harness = testing_mod.Harness.init(std.testing.allocator, .{ .width = 400, .height = 300 });
    defer harness.deinit();

    var fixture = PreviewCardFixture{ .harness = &harness };
    fixture.state = try harness.app.new(PreviewCardState, .{});
    try harness.setRoot(&fixture, PreviewCardFixture.render);
    ...
    try harness.hoverOver("preview-card-trigger");
    try std.testing.expect(!harness.app.read(PreviewCardState, fixture.state).visible);
...
test "preview card hides on Escape" {
    var harness = testing_mod.Harness.init(std.testing.allocator, .{ .width = 400, .height = 300 });
    defer harness.deinit();
...
test "preview card stays open when pointer moves onto card" {
    var harness = testing_mod.Harness.init(std.testing.allocator, .{ .width = 400, .height = 300 });
    defer harness.deinit();
    ...
    const center = harness.centerOf(element.elementId("stay-preview-card")) orelse return error.ElementNotFound;
